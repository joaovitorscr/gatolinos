...
function PillarCardRoot({ className, ...props }: ComponentProps<"article">) {
  return (
    <article
      data-slot="pillar-card"
      className={cn(
        "group rounded-[2rem] bg-white p-8 shadow-[0_32px_64px_-24px_rgba(61,38,17,0.12)] ring-1 ring-outline-variant/30 transition-transform duration-300 hover:-translate-y-1 sm:p-10",
        className,
      )}
      {...props}
    />
...
function PillarCardDescription({ className, ...props }: ComponentProps<"p">) {
  return (
    <p
      ...
      className={cn(
        "mt-4 text-base leading-8 text-on-surface-variant",
        className,
      )}
      {...props}
    />
...
export const PillarCard = Object.assign(PillarCardRoot, {
  Icon: PillarCardIcon,
  Title: PillarCardTitle,
  Description: PillarCardDescription,
});
